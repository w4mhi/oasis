...
All logic lives in gps.py (same directory); the gpsd/chrony parts are shared
with features/gps-L76X via common/gpsd_chrony.py.

Only one GPS device can discipline gpsd/chrony at a time — this feature and
features/gps-L76X are mutually exclusive. Running one after the other warns
and refuses to retarget gpsd unless you pass --force.
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog=("Examples:\n"
                ...
                "  python3 features/gps/install-gps.py --device /dev/ttyACM0\n"
                "  python3 features/gps/install-gps.py --force          # retarget gpsd from gps-L76X\n"
    ...
    ap.add_argument("--force", action="store_true",
                    help="Retarget gpsd/chrony even if features/gps-L76X already configured a "
