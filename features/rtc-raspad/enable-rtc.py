...
`bigtreetech-7in`, which writes:
    ...
    dtoverlay=vc4-kms-dsi-7inch,dsi1        (the DSI panel; usually already there)
    dtoverlay=i2c-rtc,pcf8563,i2c_csi_dsi   (the clock -> /dev/rtc0 at boot)

Both go inside this feature's own config.txt BEGIN/END block — except a line
already present elsewhere in config.txt, which is left where it is so an
uninstall can never delete it. On a working Raspad the display overlay is
normally already next to the stock vc4-kms-v3d line, so only the RTC overlay
ends up owned here.
...
def removal_record(repo_root=None):
    ...
    a pre-existing DSI display overlay survives the uninstall (deleting it would
    blank the screen)."""
