...
Usage:
  ...
  python3 scripts/install-gps.py --assist-now    # (u-blox) load AssistNow Offline for fast cold fix
...
def main():
    ap = argparse.ArgumentParser(
        ...
        epilog=("Examples:\n"
                "  python3 scripts/install-gps.py                  # autodetect GPS device\n"
                "  python3 scripts/install-gps.py --device /dev/ttyACM0\n"
                "  python3 scripts/install-gps.py --check          # report status, change nothing\n"
                "  OASIS_UBLOX_TOKEN=xxx python3 scripts/install-gps.py --assist-now  # u-blox fast-fix\n"),
    )
