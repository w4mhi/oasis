...
def detect_digirig_serial(override=None):
    """Return the DigiRig's CP210x USB-serial by-id path (its RTS line keys PTT),
    or None if the adapter isn't present. The by-id path is stable across replug
    / renumbering. An explicit override short-circuits detection."""
    if override:
        return override
    cands = sorted(glob.glob(MODEM_DIGIRIG_SERIAL_GLOB))
...
def write_digirig_config(callsign, adevice, ptt_serial, force=False):
    ...
    ptt_line = (f"PTT {ptt_serial} RTS" if ptt_serial
                else "# PTT <serial by-id> RTS  — DigiRig CP210x not detected; "
                     "re-run with --modem-ptt-serial /dev/serial/by-id/usb-Silicon_Labs_CP210x…-if00-port0")
...
MYCALL {callsign}
MODEM 1200
# DigiRig keys PTT via the CP210x's RTS line (NOT GPIO). Use the by-id path.
{ptt_line}
AGWPORT {MODEM_AGW_PORT}
KISSPORT {MODEM_KISS_PORT}
...
"""
    ...
    _ok(f"Wrote {cfg_path}")
    if not ptt_serial:
        _warn("DigiRig CP210x serial not detected — plug in the DigiRig and re-run, "
              "or set it with --modem-ptt-serial /dev/serial/by-id/usb-Silicon_Labs_CP210x…-if00-port0.")
    return cfg_path
