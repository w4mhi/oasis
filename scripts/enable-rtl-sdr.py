...
Pairs with install-rtl-sdr.py: that one installs the tools, this one tests the
...
What this does:
  1. Checks prerequisites (Linux + systemd, rtl_fm, socat, tcpdump — all
     installed by install-rtl-sdr.py)
...
Usage:
  ...
  python3 scripts/enable-rtl-sdr.py --freq 144.800M # EU APRS
  python3 scripts/enable-rtl-sdr.py --gain 28 --ppm 12
  python3 scripts/enable-rtl-sdr.py --check         # test only, no service changes
  python3 scripts/enable-rtl-sdr.py --no-enable     # write the unit but don't start it

Requires: Linux, systemd, sudo. rtl_fm + socat + tcpdump — all installed by
install-rtl-sdr.py (run that first). A dongle and 2 m antenna plugged in. Stop
...
def check_prereqs():
    ...
    # by install-rtl-sdr.py — this script only checks they're present.
    ...
    if tcpdump:
        ...
        _warn("tcpdump not found — verification step 3 (UDP capture) won't work. "
              "Install it: python3 scripts/install-rtl-sdr.py")
...
def main():
    parser = argparse.ArgumentParser(
        ...
        epilog=(
            "Examples:\n"
            "  python3 scripts/enable-rtl-sdr.py\n"
            "  python3 scripts/enable-rtl-sdr.py --freq 144.800M --gain 28 --ppm 12\n"
            "  python3 scripts/enable-rtl-sdr.py --check       # test only\n"
        ),
    )
