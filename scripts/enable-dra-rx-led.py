...
Usage:
  ...
  python3 scripts/enable-dra-rx-led.py --no-enable     # write unit, don't start
  python3 scripts/enable-dra-rx-led.py --gpio 16       # override LED GPIO (BCM)
  python3 scripts/enable-dra-rx-led.py --uninstall     # stop + remove the service
  python3 scripts/enable-dra-rx-led.py --self-test     # blink the LED 5x and exit
  python3 scripts/enable-dra-rx-led.py run             # run the daemon (used by unit)
...
"""
    ...
    if status == "active":
        ...
        _info("Verify wiring any time:  python3 scripts/enable-dra-rx-led.py --self-test")
    else:
