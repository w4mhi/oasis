...
│
├── maps/                       ← map.html + map-config.json; *.mbtiles placed here
...
| `GET /api/tilejson/<file>.mbtiles` | TileJSON 2.2 descriptor for a vector tile file |
| `GET /api/tiles/<file>.mbtiles/<z>/<x>/<y>` | Individual vector tile (gzip-compressed MVT) |
...
Vector tile maps rendered in the browser with MapLibre GL. Tiles are served from a local **MBTiles** SQLite file — no internet required at runtime.

```
Prep machine (internet, one-time)
  └─ Download *.mbtiles from MapTiler Data
       └─ drop into maps/

Pi (no internet)
  └─ server/app.py :8083
       ├─ GET /api/tilejson/<file>.mbtiles  → TileJSON descriptor
       ├─ GET /api/tiles/<file>.mbtiles/z/x/y  → gzip-compressed MVT
       └─ maps/map.html  ← MapLibre GL renders in browser
```

### Step 1 — Download an MBTiles file

Go to **[MapTiler Data](https://www.maptiler.com/data/)** and download a pre-built OpenMapTiles MBTiles file for your region. The file uses the **OpenMapTiles (OMT) v3.x** schema which OASIS expects.

> **Recommended:** download a state or regional extract (e.g. *US State — Washington*) to keep the file size manageable on a Pi SD card. Full-country or planet files can exceed several GB.

Files are licensed under ODbL (OpenStreetMap data). A free MapTiler account is required to download.

### Step 2 — Register and deploy

Place the `.mbtiles` file in `maps/` and add an entry to `maps/map-config.json` so the map opens at the right location:

```json
{
  "osm-2020-02-10-v3.11_us_washington.mbtiles": { "center": [-121.83, 47.28], "zoom": 9 }
}
```
...
```bash
scp maps/<region>.mbtiles pi@raspberrypi.local:/home/pi/oasis-emcomm/maps/
scp maps/map-config.json  pi@raspberrypi.local:/home/pi/oasis-emcomm/maps/
```

The server reads MBTiles as a SQLite database directly — no build step, no external tools, no extra Python packages (uses stdlib `sqlite3`).

### Refreshing JS libraries

MapLibre GL is bundled in `server/map-assets/`. To update:
...
| `/api/tilejson/` returns 404 | Old server still running | Stop and restart (`./start.sh`) |
| No roads / blank map | Layer filter mismatch | Open browser console — check diagnostics panel for layer names |
| `SyntaxError: Unexpected token '<'` in console | TileJSON endpoint returning HTML 404 | Server needs restart (see above) |
| Labels/fonts missing | Glyph PBF files absent | Check `server/map-assets/fonts/Open Sans Regular/` |
| Map tiles not found | `.mbtiles` file not in `maps/` | Verify filename matches `map-config.json` entry |
