...
│                                  all generated by install-fcc-database.py (gitignored)
│
...
Lookups return callsign, name, city, state, and Maidenhead grid (derived from ZIP centroid). Name and grid search require the secondary indexes built by `install-fcc-database.py`.
...
python3 scripts/install-fcc-database.py --index-only

# Force refresh zipcodes.csv even if it already exists:
python3 scripts/install-fcc-database.py --full-zip
```
...
| `zipcodes.csv` | ZIP → lat/lon — gitignored; generated from GeoNames by `install-fcc-database.py` |
...
**Copy to Pi** (if you ran `install-fcc-database.py` on a different machine):
...
If you ran `install-fcc-database.py` directly on the Pi, no copying is needed.
