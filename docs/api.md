...
| Method | Path | Auth | Params/Body | Description |
|---|---|---|---|---|
| GET | `/api/satellites` | — | — | Roster with per-sat TLE lines + `{satellites, tle_age_days, station}`. `station` is `{lat, lon, horizon, min_elev}` — `horizon` is the operator's azimuth-dependent skyline (16 compass sectors → elevation floor, `{}` when never drawn) and is served **raw**: the mask is evaluated client-side in `common/js/horizon.js`, since every pass already carries `peak_az`. `min_elev` rides along so a missing sector can fall back without a second request. Each entry in a satellite's `downlinks` is a group of transmitters sharing a (frequency, demodulator), carrying `modes`, `uplinks` (a list, possibly empty, each naming the `mode` it belongs to) and `one_way` — true only when SatNOGS types **every** member `Transmitter`, its own word for a downlink that does not listen. `one_way` exists so a consumer can tell "there is no uplink" from "we were not told one": an absent uplink alone cannot distinguish them. |
...
| POST | `/api/satellites/horizon` | **CSRF** | `{horizon:{N:25, NNE:24, …}}` | Save the operator's azimuth-dependent skyline into `station.json`, preserving every other key. Keys must be among the 16 compass sectors and values `0.0–89.0`; a **partial** mask is legal (a missing sector falls back to `min_elev`) and an empty one means clear all round. The mask **replaces** rather than merges, so a sector can be removed. `400` invalid, `500` when `station.json` exists but will not parse — it is refused rather than overwritten, since it carries the callsign, grid and position an offline station cannot re-fetch. |
