...
- **🔄 Data updates** — perishable data (satellite TLEs, SatNOGS transmitters, and the FCC callsign database) refreshes itself whenever the internet is reachable and fails silently when it is not, so a station can move from the bench to a summit and back without anyone thinking about it. Data age is always on screen — a header pill, a kiosk chip, and a **Data Updates** section on Diagnostics that says what was updated and when, what is missing, and what you need to do. Large downloads wait for an unmetered link or a tap, and data OASIS does not fetch (the RepeaterBook CSV) is reported by age so you know to re-export it. → [Setup](docs/SETUP.md#automatic-data-updates)
