...
> 🔢 **Version-aware & idempotent:** every `install-*` script installs when absent, **upgrades** when newer, and **keeps what you have** otherwise — never downgrades. Re-running setup, or installing from an older USB bundle, can't clobber a newer package.
...
| `install-rtl-sdr.py` | Installs RTL-SDR tools (+ socat/tcpdump), blacklists the DVB driver | On the Pi, for USB SDR dongles | Raspberry Pi / Debian | ⚠️ bundled `.deb` if present |
...
| `enable-autostart-pi.py` | Installs a **systemd service** so OASIS starts on boot; `--with-browser` adds a Chromium kiosk; `--disable` removes both | After first successful manual run | Raspberry Pi OS (systemd) | ❌ offline |
