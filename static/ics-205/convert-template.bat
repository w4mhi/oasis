...
REM Converts an ICS 205 AcroForm PDF into the base64 JS template used by
REM ics-205.html at runtime.
REM
REM Usage:
REM   convert-template.bat                        (uses default: ics_forms_205.pdf)
REM   convert-template.bat path\to\my-form.pdf
REM
REM Output: ics205-template.js  (overwrites; previous version backed up)
REM ────────────────────────────────────────────────────────────────────────────

setlocal enabledelayedexpansion

if "%~1"=="" (
    set "PDF=ics_forms_205.pdf"
) else (
    set "PDF=%~1"
)

set "OUT=ics205-template.js"
set "BACKUP=ics205-template.js.bak"

REM ── Check PDF exists ─────────────────────────────────────────────────────────
if not exist "%PDF%" (
    echo ERROR: PDF file not found: %PDF%
    echo Usage: convert-template.bat [path\to\ics_forms_205.pdf]
    exit /b 1
)
...
open(out,'w',encoding='utf-8').write('// ICS 205 PDF Template -- embedded as base64\n// Source : '+src+'\n// Generated: '+ts+'\n// The variable ICS205_PDF_B64 is loaded by ics-205.html at runtime.\nconst ICS205_PDF_B64 = \\''+b64+'\\';\n'); ^
