...
# ────────────────────────────────────────────────────────────────────────────
# convert-template.sh
# Converts an ICS 309 AcroForm PDF into the base64 JS template used by
# ics-309.html at runtime.
#
# Usage:
#   ./convert-template.sh                        # uses default: ics_forms_309.pdf
#   ./convert-template.sh path/to/my-form.pdf    # use a specific file
#
# Output: ics309-template.js  (overwrites; previous version backed up)
# ────────────────────────────────────────────────────────────────────────────

set -euo pipefail

PDF="${1:-ics_forms_309.pdf}"
OUT="ics309-template.js"
BACKUP="${OUT}.bak"

# ── Checks ────────────────────────────────────────────────────────────────────
if [[ ! -f "$PDF" ]]; then
  echo "ERROR: PDF file not found: $PDF"
  echo "Usage: $0 [path/to/ics_forms_309.pdf]"
  exit 1
fi
...
with open(out_path, 'w', encoding='utf-8') as out:
    out.write(f'// ICS 309 PDF Template — embedded as base64\n')
    out.write(f'// Source : {source}\n')
    out.write(f'// Generated: {ts}\n')
    out.write(f'// The variable ICS309_PDF_B64 is loaded by ics-309.html at runtime.\n')
    out.write(f"const ICS309_PDF_B64 = '{b64}';\n")
