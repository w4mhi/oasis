...
|----|----------|-------|----------------|
| **A** | SETUP.md — Guided setup + Server setup | `setup-oasis.py` vs `setup-server.py` — the relationship is never stated. Both sections read as standalone procedures. A user will run both and wonder if that causes a problem. | Add one sentence at the top of **Server setup**: *"If you used the guided menu above, skip this section — the menu already runs `setup-server.py` for you."* |
