...
FEATURES = [
    ...
    Feature("nwr", "NOAA Weather Radio (SAME/EAS)", "services/nwr/install.py",
            "Decode official National Weather Service alerts from a 162 MHz weather channel on an RTL-SDR: event code, counties, map marker and a spoken announcement. Manual - you start a listening session, so it shares the dongle instead of holding it. Needs internet (apt) for multimon-ng.",
            "Server", default=False, needs=["server", "rtl-sdr"], internet=True,
            recommend="Assign an RTL-SDR to 'nwr' in Setup - Hardware, then open the Weather Radio page and press Scan to find your local channel."),
