...
FEATURES = [
    ...
    Feature("openwebrx", "OpenWebRX (SDR monitor) — experimental", "services/openwebrx/install.py",
            "Browser SDR receiver/decoder on :8073 (RX only: voice/CW/RTTY/FT8/…). Installed OFF by default and NOT managed by OASIS — start it yourself with systemctl. Needs internet (3rd-party apt repo).",
            "Server", default=False, needs=["server"], internet=True,
            recommend="OpenWebRX is OFF by default and unmanaged: `sudo systemctl start openwebrx`, then open :8073. It seizes an RTL-SDR, so stop the APRS feed / ADS-B / weather watch first."),
    Feature("adsb", "ADS-B Aircraft", "services/adsb/install.py",
            ...
            "Server", default=False, needs=["server"], internet=True,
            recommend="ADS-B is OFF by default — start it from the dashboard (it offers to stop GrayWolf + OpenWebRX + the SDR feed first). Aircraft appear on the offline map."),
    Feature("nwr", "NOAA Weather Radio (SAME/EAS)", "services/nwr/install.py",
            "Decode official National Weather Service alerts from a 162 MHz weather channel on an RTL-SDR: event code, counties, map marker and - once you switch the bell on - a spoken announcement. Assigning a dongle to 'nwr' starts an always-on watch (the oasis-nwr service) that picks the strongest channel itself and holds that dongle until you release it. Needs internet (apt) for multimon-ng.",
            "Server", default=False, needs=["server", "rtl-sdr"], internet=True,
            recommend="Assign an RTL-SDR to 'nwr' in Setup - Hardware and the watch starts on its own, from boot; it finds your local channel. On the Weather Radio page: pin a channel, listen in, read what decoded, choose your counties, and switch on the spoken bell (off by default, silent overnight)."),
    Feature("service-controls", "Dashboard service controls", "enable-service-controls.py",
            "Grant a scoped sudoers rule so the dashboard start/stop buttons work (no passwords in the browser). Needed to start/stop GrayWolf, Kiwix and the rest from the dashboard.",
            "Server", default=True, needs=["server"],
            recommend="Dashboard power buttons enabled (start/stop GrayWolf · Kiwix · Winlink · APRS feed)."),
