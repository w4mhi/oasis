...
FEATURES = [
    ...
    Feature("nwr", "NOAA Weather Radio (SAME/EAS)", "services/nwr/install.py",
            "Decode official National Weather Service alerts from a 162 MHz weather channel on an RTL-SDR: event code, counties, map marker and a spoken announcement. Assigning a dongle to 'nwr' starts an always-on watch (the oasis-nwr service) that picks the strongest channel itself and holds that dongle until you release it. Needs internet (apt) for multimon-ng.",
            "Server", default=False, needs=["server", "rtl-sdr"], internet=True,
            recommend="Assign an RTL-SDR to 'nwr' in Setup - Hardware and the watch starts on its own, from boot; it finds your local channel. The Weather Radio page pins a channel, listens in, and shows what decoded."),
