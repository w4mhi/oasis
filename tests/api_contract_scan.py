...
def _jsonify_dict(node):
    """If `node` is `jsonify({...})`, return the dict node, else None."""
    ...
    if name != "jsonify" or not node.args:
        return None
    return node.args[0] if isinstance(node.args[0], ast.Dict) else None
...
def returns_of(fn):
    ...
    {"line", "status", "has_ok", "ok_value", "has_error", "keys"}
