...
test('fmtAge buckets + classes', () => {
  ...
  // Sub-minute ages now show seconds (freshness matters for recent traffic).
  assert.deepStrictEqual(F.fmtAge(agoIso(3 * 1000)), { text: '3s ago', cls: 'age-ok' });
  assert.deepStrictEqual(F.fmtAge(agoIso(59 * 1000)), { text: '59s ago', cls: 'age-ok' });
  assert.deepStrictEqual(F.fmtAge(agoIso(60 * 1000)), { text: '1m ago', cls: 'age-ok' });
...
test('fmtLastHeard guards + shape', () => {
  ...
  const out = F.fmtLastHeard('2026-07-17T14:05:09Z');
  // UTC portion is timezone-independent — assert it exactly (now with seconds).
  assert.ok(out.startsWith('2026-07-17 14:05:09Z'));
